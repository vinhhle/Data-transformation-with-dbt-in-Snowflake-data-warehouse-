  ...
  create or replace  view olist.olist_raw.fact_orders
  
   as (
    

WITH orders AS (
    SELECT * FROM olist.olist_raw.stg_orders
),
items AS (
    SELECT * FROM olist.olist_raw.stg_order_items
),
products AS (
    SELECT * FROM olist.olist_raw.stg_products
)
SELECT 
    orders.order_id,
    orders.customer_id,
